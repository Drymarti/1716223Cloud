...
plt.style.use('ggplot')app = Flask(__name__)
 
 @app.route('/')
def index():
    analysis_details = analyze()
    fans_by_housemate, fans_by_location, followers_of_fans_by_hm = analysis_details
    fans_by_location_table = fans_by_location.pivot_table(index='location', columns='housemate_name', values='count')    return render_template("index.html", fans_by_housemate_df=fans_by_housemate.to_html(), 
...
def fans_by_location_bar_chart():
    ...
    fig2.savefig(img)
    img.seek(0)
    return send_file(img, mimetype='image/png')if __name__ == "__main__":
    app.run()